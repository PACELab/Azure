    ...
    def execute(self, tup, run_delayed_tasks=True, can_delay=True, original_time_stamp=None):
        ...
        if run_delayed_tasks == True:
            ...
            while (y!=None and y<=time_stamp):
                del_req = heappop(self.delayed_heap)  #(time_stamp,task_id, vm_id, num_cores, ram_needed, c_d,orig_time_stamp)
        ...
        if int(c_d) == 1:  # c inidcates create request
            ...
            if self.delayed_vm_reqs:
                while self.num_cores_used+float(self.delayed_vm_reqs[0][3])<=self.core_hard_limit:
                    ...
                    temp_tup = (waiting_tup[0], waiting_tup[1], time_stamp, waiting_tup[3], waiting_tup[4], waiting_tup[5])
                    orig_time_stamp = float(waiting_tup[6])
                    self.execute(temp_tup, run_delayed_tasks=False, can_delay=False, original_time_stamp=orig_time_stamp)
                    if not self.delayed_vm_reqs:
                        break
