...
"delay_time" needs to be set, which would indicate the time by which the vm is to be delayed when the time stamp is multiple of delay_step.
"delay_step" needs to be set, when the time stamp is multiple of delay_step, all vms at that time stamp would be delayed.

In this algorithm, vm would be delayed only by "delay_time" and when the timestamp is a multiple of delay_step.
...
class Algorithm(object):
    def __init__(self):
        ...
        self.delay_step = float(config["delay_step"])
    ...
    def execute(self, tup,run_delayed_tasks=True,can_delay=True,original_time_stamp=None):
        ...
        if c_d.strip() == "c":  #c inidcates create request
            ...
            if can_delay==True and time_stamp>0:
                if time_stamp%self.delay_step == 0:
