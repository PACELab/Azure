...
class Algorithm(object):
    ...
    def execute(self, tup, run_delayed_tasks=True, can_delay=True, original_time_stamp=None, migrate=False):
        ...
        if time_stamp==2836546229.0:
            print tup
        if time_stamp==2836391759.0:
            print "****",tup,run_delayed_tasks,original_time_stamp
        if not self.flag:
        ...
        if run_delayed_tasks == True:
            while True:
                ...
                if self.delayed_vm_reqs:
                    if time_stamp - self.delayed_vm_reqs[0][-1] >= self.min_delay and (self.num_cores_used + float(self.delayed_vm_reqs[0][3]) <= self.core_hard_limit):
        ...
        if int(c_d) == 1:  # c inidcates create request
            ...
            if (can_delay == True) and ((self.num_cores_used + num_cores > self.core_hard_limit)or(self.num_cores_used>7000 )):
                if original_time_stamp == None:
                    ...
                    delayed_tup = (
                    task_id, orig_vm_id, time_stamp + self.delay_time, num_cores, ram_needed, c_d, original_time_stamp)
            ...
            if self.delayed_vm_reqs:
                ...
                if num_cores>=float(self.delayed_vm_reqs[0][3]) or time_stamp - self.delayed_vm_reqs[0][-1]>= self.min_delay:
                    flag=True
                else:
                    flag=False
