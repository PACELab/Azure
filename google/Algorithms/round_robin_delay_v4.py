...
The condition to check if the VM needs to be delayed or not depends on the avg+std_dev or avg+2*std_dev. avg and std_dev are calculated from the past "window length of the effective cores used."
    ...
    def execute(self, tup,run_delayed_tasks=True,can_delay=True,original_time_stamp=None):
        ...
        if self.delay_time_stamp <= time_stamp and run_delayed_tasks==True and self.delay_time_stamp!=None:
            ...
            while x<=time_stamp and x!=None:
                ...
                temp_tup = (waiting_tup[0],waiting_tup[1],waiting_tup[2],waiting_tup[3],waiting_tup[4],waiting_tup[5],waiting_tup[6])
                if waiting_tup[6].strip()=="d":
                    orig_time_stamp = float(waiting_tup[7])
                    if (time_stamp-orig_time_stamp)<self.max_delay_time:
                        self.execute(temp_tup,run_delayed_tasks=False,original_time_stamp=orig_time_stamp,can_delay=True)
                    else:
                        self.execute(temp_tup,run_delayed_tasks=False,original_time_stamp=orig_time_stamp,can_delay=False)
                else:
                    orig_time_stamp = float(waiting_tup[7])
                    if (time_stamp-orig_time_stamp)<self.max_delay_time:
                        self.execute(temp_tup,run_delayed_tasks=False,original_time_stamp=orig_time_stamp,can_delay=True)
        ...
        if c_d.strip() == "c":  #c inidcates create request
            ...
            if can_delay==True and len(self.effective_cores_lst)>self.window_size+2 and prev_timestamp!=None:
                if prev_timestamp != time_stamp:
                  last_window_lst = self.effective_cores_lst[-1:-int(self.window_size+1):-1]
                else:
                  last_window_lst = self.effective_cores_lst[-2:-int(self.window_size+2):-1]
                np_arr = np.array(last_window_lst)
                avg_val = np.average(np_arr)
                
                if avg_val>(self.multiplier*self.max_effective_cores):
                    if original_time_stamp==None:
                        delayed_tup = (vm_id, time_stamp+self.delay_time, num_cores, ram_needed, avg_cpu, p95maxcpu, c_d, time_stamp)
                    else:
                        delayed_tup = (vm_id, time_stamp+self.delay_time, num_cores, ram_needed, avg_cpu, p95maxcpu, c_d, original_time_stamp)
            ...
            for server_pool_type, value in config["servers"]["types"].iteritems():
                ...
                for _ in xrange(number_of_servers):
                    ...
                    if self.allocation_dict[server_pool_type][
                        ...
                        self.max_effective_cores = max(self.max_effective_cores,self.effective_cores_used)
            ...
            if can_delay==True and vm_id in self.delayed_vms:
                if self.vm_delay_time[vm_id]==0:
                    if original_time_stamp==None:
                        delayed_del_tup = (vm_id, time_stamp+self.delay_time, num_cores, ram_needed, avg_cpu, p95maxcpu, c_d, time_stamp)
                    else:
                        delayed_del_tup = (vm_id, time_stamp+self.delay_time, num_cores, ram_needed, avg_cpu, p95maxcpu, c_d, original_time_stamp)
                    if not self.delayed_vm_reqs:
                        self.delay_time_stamp = time_stamp+self.delay_time
    ...
    def final(self):
        ...
        while x!=None:
            ...
            if waiting_tup[6].strip()=="d":
                ...
                orig_time_stamp = float(waiting_tup[7])
                if (x-orig_time_stamp)<self.max_delay_time:
                    self.execute(temp_tup,False,orig_time_stamp,True)
                else:
                    self.execute(temp_tup,False,orig_time_stamp,False)
            if self.delayed_vm_reqs:
                x = self.delayed_vm_reqs[0][1]
            else:    
                x = None
        ...
        self.net_cores_dict.pop(self.stats_time[-1])
        self.net_ram_dict.pop(self.stats_time[-1])
