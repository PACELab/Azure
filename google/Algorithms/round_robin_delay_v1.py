...
"""
This doesnot consider the max delay time parameter.

"delay_time" needs to be set, which would indicate the time by which the vm is to be delayed
"window size" needs to be set, which would indicate the window length of which the mean and std dev would be calculated to find the limit
"stdMultiplier" needs to be set, which would indicate the factor with which std dev would be multiplied with.

In this algorithm, vm would be delayed only once by "delay_time"

If, the VM is delayed and the delay time expires, the would be deployed even if it doesnot satisfy the mean+stdmultiplier*stddev limit
    ...
    def execute(self, tup,force_operate=False):
        ...
        if c_d.strip() == "c":  #c inidcates create request
                        ...
                        if len(self.num_cores)>self.window_size+1 and force_operate==False:
                            ...
                            if (core_mean+(core_std_dev*self.stdmultiplier)< self.num_cores_used+num_cores) or (
                              ram_mean+(ram_std_dev*self.stdmultiplier)< self.ram_used+ram_needed):
                                ...
                                delayed_tup = (vm_id, time_stamp+self.delay_time, num_cores, ram_needed, avg_cpu, p95maxcpu, c_d)
            ...
            if force_operate==False and vm_id in self.delayed_vms:
                delayed_del_tup = (vm_id, time_stamp+self.delay_time, num_cores, ram_needed, avg_cpu, p95maxcpu, c_d)
                if not self.delayed_vm_reqs:
                    self.delay_time_stamp = time_stamp+self.delay_time
