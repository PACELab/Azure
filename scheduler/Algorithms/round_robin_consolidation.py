...
import dill
from scheduler.Algorithms.consolidation import Consolidation
...
class Algorithm(object):
    ...
    def consolidate(self,time_stamp):
        Consolidation(allocation_dict=self.allocation_dict, time_stamp=time_stamp).execute()
        self.count_servers_used()
    ...
    def count_servers_used(self):
        ...
        for pool in self.allocation_dict.keys():
            ...
            for server_num in pool_value.keys():
                ...
                if float(self.allocation_dict[pool][server_num].num_cores_left) == float(config["servers"]["types"][pool][
                    "max_cores_per_server"]) and float(self.allocation_dict[pool][
                    server_num].ram_left) == float(config["servers"]["types"][pool]["max_ram_per_server"]):
                    del self.allocation_dict[pool][server_num]
                else:
                    self.servers_used += 1
    ...
    def execute(self, tup):
        f=0
        self.count_servers_used()
        # print "servers used is {num}".format(num=self.servers_used)
        vm_id, time_stamp, num_cores, ram_needed, c_d = tup
        #print "vm id is",vm_id
        ...
        if not self.allocation_dict:
            self.allocation_dict["B"][1] = Allocation(type="B")
            self.allocation_dict["B"][1].num_cores_left -= num_cores
            self.allocation_dict["B"][1].ram_left -= ram_needed
            self.allocation_dict["B"][1].vms.append((vm_id, num_cores, ram_needed))
            f = 1
        self.consolidate(time_stamp = time_stamp)
        if c_d.strip()   == "c" and not f:
            if vm_id in self.ignore_vms_list:
                return
            for server_pool_type in config["servers"]["types"].keys():
                value = config["servers"]["types"][server_pool_type]
                ...
                for _ in xrange(number_of_servers):
                    ...
                    if self.allocation_dict[server_pool_type][
                        ...
                        self.allocation_dict[server_pool_type][value["server_number"]].vms.append(
                            (vm_id, num_cores, ram_needed))
                        # print "Creating VM: " + vm_id + " on Server: ", server_tup, "Available Resources on Server after creation is: ", \
                        #     self.allocation_dict[server_pool_type][value["server_number"]].ram_left, " RAM ", \
                        #     self.allocation_dict[server_pool_type][value["server_number"]].num_cores_left, " Cores"
                        ...
                        #print "servers used is",self.servers_used
            ...
            l = self.allocation_dict[pool_type][server_num].vms
            self.allocation_dict[pool_type][server_num].vms = [i for i in l if i[0] != vm_id]
            # print "Deleting VM: " + vm_id + " from Server: ", data, "Available Resources on Server after deletion is: ", \
            #     self.allocation_dict[pool_type][server_num].ram_left, " RAM ", self.allocation_dict[pool_type][
            #     server_num].num_cores_left, " Cores"
            self.num_cores_used -= num_cores
            self.num_cores.append(self.num_cores_used)
            self.num_cores_time.append(time_stamp)
            self.ram_used -= ram_needed
    ...
    def write_to_file(self, pool_type):
        if config["mode"] == "debug":
            ...
            with open(output_file_path, "a") as f:
                s = ""
                for server_pool_type in config["servers"]["types"]:
                    value = config["servers"]["types"][server_pool_type]
